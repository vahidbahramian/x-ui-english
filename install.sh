...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/vahidbahramian/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/vahidbahramian/x-ui-english/main/config/version >/dev/null 2>&1)
        ...
        last_version=$1
        url="https://github.com/vahidbahramian/x-ui-english/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
